    ...
    >
      <div className="relative bg-card rounded-2xl shadow-2xl overflow-hidden border border-border" onClick={handleClick}>
        ...
        <div className="p-5 space-y-3">
          ...
          <p className="text-xs text-center text-muted-foreground/60 italic">Tap for details & related books</p>
        </div>
      </div>

      <div className="flex justify-center gap-6 mt-5">
        <button onClick={onSwipeLeft} className="w-14 h-14 rounded-full bg-card border-2 border-destructive/30 flex items-center justify-center shadow-lg hover:bg-destructive/10 transition-colors">
          <X className="h-6 w-6 text-destructive" />
        </button>
        <button onClick={onSwipeRight} className="w-14 h-14 rounded-full bg-card border-2 border-sage/30 flex items-center justify-center shadow-lg hover:bg-sage/10 transition-colors">
